...
class TowerGroup:
    ...
    def upgrade(self):
        ...
        #判斷串列裡每一個tower是否有被點到 並且不超過5等
        for tw in self.__towers:
            if tw == self.selected_tower and tw.level < 5:
                tw.level += 1  #提高tower等級
    ...
    def sell(self):
        ...
        #判斷串列裡每一個tower是否有被點到
        for tw in self.__towers:
            if tw == self.selected_tower:
                self.__towers.remove(tw)    #從串列中移除
                self.selected_tower = None  #移除選取的狀態
